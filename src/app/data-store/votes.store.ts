...
export class VotesStore {
  ...
  async saveStatsToServer() {
    // userID is the primary key here
  ...
  public async addGameStats(isWin: boolean, isDem: boolean, votes: number) {
    ...
    if (votes > 374) {
      this.unlockRecord('record26');
    }
    if (votes > 349) {
      this.unlockRecord('record25');
    }
    if (votes === 270) {
    ...
    if (isWin) {
      ...
      if (isDem) {
        ...
        //MN, IA, WI, MI, OH and PA 19
        if (this.Minnesota.leansDem > this.Minnesota.leansRep &&
            this.Iowa.leansDem > this.Iowa.leansRep &&
            this.Wisconsin.leansDem > this.Wisconsin.leansRep &&
            this.Michigan.leansDem > this.Michigan.leansRep &&
            this.Ohio.leansDem > this.Ohio.leansRep &&
            this.Pennsylvania.leansDem > this.Pennsylvania.leansRep)
        {
          this.unlockRecord('record19');
        }

        //NH, ME, NJ, MA, CT, VT, NY and DE 20
        if (this.NewHampshire.leansDem > this.NewHampshire.leansRep &&
          this.Maine.leansDem > this.Maine.leansRep &&
          this.NewJersey.leansDem > this.NewJersey.leansRep &&
          this.Massachusetts.leansDem > this.Massachusetts.leansRep &&
          this.Connecticut.leansDem > this.Connecticut.leansRep &&
          this.Vermont.leansDem > this.Vermont.leansRep &&
          this.NewYork.leansDem > this.NewYork.leansRep &&
          this.Delaware.leansDem > this.Delaware.leansRep)
        {
          this.unlockRecord('record20');
        }
        //AZ, NV, CO, and NM 21
        if (this.Arizona.leansDem > this.Arizona.leansRep &&
          this.Nevada.leansDem > this.Nevada.leansRep &&
          this.Colorado.leansDem > this.Colorado.leansRep &&
          this.NewMexico.leansDem > this.NewMexico.leansRep)
        {
          this.unlockRecord('record21');
        }
        //FL, GA, SC, NC, VA, TN, AL, MS, AR, and LA 22
        if (this.Florida.leansDem > this.Florida.leansRep &&
          this.Georgia.leansDem > this.Georgia.leansRep &&
          this.SouthCarolina.leansDem > this.SouthCarolina.leansRep &&
          this.NorthCarolina.leansDem > this.NorthCarolina.leansRep &&
          this.Virginia.leansDem > this.Virginia.leansRep &&
          this.Tennessee.leansDem > this.Tennessee.leansRep &&
          this.Alabama.leansDem > this.Alabama.leansRep &&
          this.Mississippi.leansDem > this.Mississippi.leansRep &&
          this.Arkansas.leansDem > this.Arkansas.leansRep &&
          this.Louisiana.leansDem > this.Louisiana.leansRep)
        {
          this.unlockRecord('record22');
        }
